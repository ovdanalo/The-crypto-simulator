...
const DevInfo = (props) => {
    ...
    return (
        <div className={`flex items-center justify-center md:w-8/12 min-h-dev  rounded-full my-4 overflow-hidden `+ props.position}>
            <div className={`flex w-40 xl:h-48 h-40 p-4 ${isDarkTheme ? 'bg-black-400' : 'bg-white-mode-200'} `+ props.roundedImg} >
                <img src={props.img} alt='' className='object-contain'></img>
            </div>
            <div className={`flex flex-col w-3/4 xl:h-48 h-40 ${isDarkTheme ? 'bg-black-400' : 'bg-white-mode-200'} `+ props.roundedDiv + ' ' + props.align}>
